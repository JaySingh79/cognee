...
def custom_openapi():
    ...
    try:
        ...
        logger.error("Failed to generate OpenAPI schema: %s", exc, exc_info=True)
        # Fallback minimal schema so /openapi.json never hangs or 500s due to
        # incompatible route annotations (e.g. Annotated[UploadFile] / Union[UUID,Literal]).
        fallback = {
            "openapi": "3.0.0",
            "info": {"title": "Cognee API", "version": "1.0.0"},
            "paths": {},
        }
        # If a cached schema already exists return it, otherwise expose fallback.
        if app.openapi_schema:
            return app.openapi_schema
